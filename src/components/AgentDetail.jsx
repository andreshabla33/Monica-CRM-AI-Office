...
export default function AgentDetail({ agent, state, extra, onClose, onStateChange }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 sm:p-6" onClick={onClose}>
      <div
        className="bg-[#0a0a14]/95 backdrop-blur-md border border-[#2a2a4e] rounded-xl w-full max-w-[500px] max-h-[90vh] overflow-hidden shadow-[0_0_30px_rgba(0,0,0,0.8)] flex flex-col font-mono relative"
      ...
      >
        ...
        <div className="flex flex-col shrink-0">
          <div className="flex items-start justify-between px-6 py-5 relative overflow-hidden">
            ...
            <button onClick={onClose} className="absolute top-4 right-4 text-[#4a4a6a] hover:text-white text-2xl w-8 h-8 flex items-center justify-center transition-colors z-20 rounded hover:bg-[#1a1a2e]">
              ×
            </button>
          </div>

          {/* Navigation Tabs */}
          <div className="flex px-6 gap-6 border-b border-[#1a1a2e] relative">
            <button 
              className={`pb-3 text-[11px] font-bold tracking-widest transition-colors relative ${
        ...
        <div className="overflow-y-auto flex-1 p-6 space-y-6 custom-scrollbar">
          {activeTab === 'metrics' ? (
            <>
              ...
              <div className="bg-[#050508]/80 border border-[#1a1a2e] rounded-xl p-5 shadow-inner">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                  ...
                  {!isOff && (
                    <button 
                      onClick={togglePause}
                      disabled={isUpdating}
                      className={`w-full sm:w-auto px-5 py-2.5 text-[10px] font-bold tracking-wider rounded-md transition-all duration-300 border ${
                ...
                {isPaused && (
                  <div className="text-[11px] text-[#e74c3c] mt-4 flex items-start gap-3 bg-[#e74c3c]/10 p-3 rounded-md border border-[#e74c3c]/30">
              ...
              <div className="space-y-3">
                ...
                <div className="bg-[#050508]/80 border border-[#1a1a2e] rounded-xl p-5">
                  ...
                  <div className="flex justify-between text-[9px] text-[#4a4a6a] mt-3 font-semibold">
                    <span>0</span>
                    <span>7.5K</span>
                    <span>15K+</span>
                  </div>
              ...
              <div className="space-y-3">
                ...
                {/* Load Bar */}
                <div className="bg-[#050508]/80 p-5 rounded-xl border border-[#1a1a2e] mt-3">
                  <div className="flex justify-between items-end mb-3">
                    <span className="text-[10px] text-[#4a4a6a] font-bold tracking-widest">WORKLOAD LEVEL</span>
                    <span className="text-[11px] font-bold bg-[#1a1a2e] px-2.5 py-1 rounded-md" style={{ color: getLoadColor(extra?.convsActive5min || 0, 10) }}>
              ...
              <div className="space-y-3">
                ...
                <div className="bg-[#050508]/80 rounded-xl border border-[#1a1a2e] p-5 space-y-3">
            ...
            <div className="bg-[#000] border border-[#1a1a2e] rounded-xl p-5 font-mono min-h-[400px] flex flex-col shadow-inner relative overflow-hidden">
              <div className="absolute top-0 left-0 w-full h-[1px] bg-gradient-to-r from-transparent via-[#a29bfe]/30 to-transparent"></div>
              
              <div className="text-[11px] text-[#a29bfe] mb-5 flex items-center gap-3 border-b border-[#1a1a2e] pb-4 tracking-widest font-bold">
              ...
              <div className="space-y-5 flex-1 pr-2">
                {!extra?.thought_traces || extra.thought_traces.length === 0 ? (
                  <div className="text-xs text-[#4a4a6a] italic flex items-center justify-center h-40 bg-[#050508]/50 rounded-lg border border-[#1a1a2e] border-dashed">
                    No hay logs de ejecución recientes.
                  </div>
                ) : (
                  extra.thought_traces.map((trace, i) => (
                    <div key={i} className="text-[11px] border-l-2 border-[#2a2a4e] pl-5 py-3 bg-[#050508]/30 rounded-r-lg hover:bg-[#0a0a14] transition-colors">
                      <div className="text-[#8a8aaa] mb-3 flex items-center justify-between">
                      ...
                      {trace.tools && (
                        <div className="mt-4 p-4 bg-[#0a0a14] rounded-md border border-[#1a1a2e] text-[#a29bfe] text-[11px] overflow-x-auto shadow-inner">
